...
A mage practises a **school** — a discipline that is itself **content** (`content/magic/*.yaml`), not code. Each school declares a `signature` (the active effect it casts on neighbours) and a ladder of named `spells` unlocked by growing `mastery`. The six shipped: Elementalism→`bolt` (blast a marauding beast), Restoration→`heal` (mend the wounded), Divination→`inspire` (hearten the low), Conjuration→`sustain` (feed the hungry), and the M26 s2 **battle magic** — Abjuration→`ward` (shield an endangered ally with temporary combat soak) and Maleficence→`curse` (hex a beast so it strikes weaker). Every `signature`/`effect` is a **spell-effect tag** whose behaviour lives once in tested code (`src/magic/effects.ts` lists the known tags; the `MagicSystem` implements each), and the loader **fail-loud cross-checks** every tag has an implementation — the same data/behaviour boundary used for capability and world-event effects. So adding a school or retuning a spell ladder is a content edit; inventing a wholly new effect (a summons, a weather-call) is one tag in `effects.ts` + a `MagicSystem` branch + content. Wards and curses are short-lived `Ward`/`Curse` components read by the combat path and swept on expiry by the `MagicSystem`.
