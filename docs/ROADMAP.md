...
- [~] **Supply/demand market:** prices move with supply/demand; agents buy food/goods; business revenue from **real sales**; businesses can **go bankrupt**.
  ...
  - [x] **Slice 2a — real sales revenue.** *(S55: food businesses (farms) now earn from **real household demand** — `MarketSystem` distributes the day's farm-food spend (`min(farmSupply,demand) × price`) across food producers by workforce share, and `EconomySystem` withholds the synthetic per-worker revenue from `producesFood` businesses (other trades keep it until their goods get markets). So a farm's balance tracks the food economy. Pure arithmetic (no RNG → soak byte-identical, deterministic). Legible: a "Farm sales" figure + 🌾 markers in the Economy tab. +5 `test/market.test.ts`. D45.)*
  - [ ] **Slice 2b — business turnover (bankruptcy + founding).** Let a persistently-unprofitable business **fail** and a new one open. **Finding (D45):** built + tested + removed this session because it's *structurally dormant* — per-worker-share sales keep every staffed farm proportionally profitable, so failure never fires (verified across the default soak + a forced-glut stress run, zero closures). Needs a **fixed operating cost** per business + a **population-scaled business count** (so reopening doesn't churn empty businesses early-game) to make failure a real, occasional outcome.
