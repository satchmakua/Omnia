...
## ▶ Milestone 10 — Causal & Legible Minds (the two loops)  *(next — reprioritised 2026-06-17)*
...
- [ ] **Slice 4.5 — Conversation/Language tabs + visible chat (legibility half of slice 4).** **Conversation tab** (active + past dialogue across the town) and **Language tab** (per-tongue phoneme inventory, sample words & structure, sound-change history, fluency/bilingualism spread) as master tabs. Make agents *visibly* chat — decouple the "chatting" badge from the rarely-low social need so adjacency-driven dialogue (the `❝` lines) reads on the map. *(Also: a bilingualism/mutual-intelligibility metric in `analysis/metrics.ts` — language shifting a measurable town-level outcome, parallel to slice 3.5's metrics. Split out of slice 4 in S43 to keep the causal mechanic one clean session; mirrors the slice 3 → 3.5 mechanic/legibility split.)*

**DoD:** culture (all axes), beliefs, and language each **measurably change behaviour** (a test shows each shifting an outcome distribution — ✓ all three as of S43); the master tabbed view holds every view incl. Conversation + Language + Mind (4.5); the sleep/work jitter is gone; all legible; determinism + soak hold.

## Milestone 11 — Homes & Property  *(pulled from old M12; requested now)*
